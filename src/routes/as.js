...
router.post('/as/:table/:id', async (req, res) => {
    ...
    const { columns, values } = req.body;
    try {
        const { rows: Data } = await asClient.query(`UPDATE ${tableName} SET (${columns}) = (${values}) WHERE id = ${id} RETURNING *`);
        res.json(Data);
    } catch (err) {
        res.status(500).send(err);
    }
...
router.post('/as/:table/:id', async (req, res) => {
    ...
    const id = req.params.id;
    try {
        const { rows: Data } = await asClient.query(`DELETE FROM ${tableName} WHERE id = ${id} RETURNING *`);
        res.json(Data);
    } catch (err) {
        res.status(500).send(err);
    }
