...
router.post('/:table', async (req, res) => {
    ...
    try {
        const { rows: Data } = await naClient.query(`INSERT INTO ${tableName} (${columns.join(', ')}) VALUES (${stringifyAndJoin(values)}) RETURNING *`);
        res.json(Data);
    } catch (err) {
        res.status(500).send(err);
    }
...
router.put('/:table/:id', async (req, res) => {
    ...
    const { columns, values } = req.body;
    try {
        const { rows: Data } = await naClient.query(`UPDATE ${tableName} SET (${columns.join(', ')}) = (${stringifyAndJoin(values)}) WHERE ${tableName.slice(0, -1)}id = ${id} RETURNING *`);
        res.json(Data);
    } catch (err) {
        res.status(500).send(err);
    }
